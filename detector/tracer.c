 ...
 *   blr Xn    : 11010110 0011 1111 0000 00 Rn 00000  → (insn & 0xFFFFFC1F) == 0xD63F0000
 *   br  Xn    : 11010110 0001 1111 0000 00 Rn 00000  → (insn & 0xFFFFFC1F) == 0xD61F0000
 *   ret Xn    : 11010110 0101 1111 0000 00 Rn 00000  → (insn & 0xFFFFFC1F) == 0xD65F0000
 */
static insn_kind_t decode_branch(uint32_t insn) {
    if ((insn >> 26) == 0x25)                     return INSN_BL;
    if ((insn & 0xFFFFFC1FU) == 0xD63F0000U)      return INSN_BLR;
    if ((insn & 0xFFFFFC1FU) == 0xD61F0000U)      return INSN_BR;
    if ((insn & 0xFFFFFC1FU) == 0xD65F0000U)      return INSN_RET;
    return INSN_OTHER;
}
...
static int plant_breakpoint(pid_t pid, uint64_t addr, uint32_t *orig_out) {
    ...
    *orig_out = (uint32_t)((uint64_t)word & 0xFFFFFFFFULL);
    uint64_t patched = ((uint64_t)word & 0xFFFFFFFF00000000ULL) | (uint64_t)BRK_INSN;
    if (ptrace(PTRACE_POKETEXT, pid, (void *)(uintptr_t)addr,
               (void *)(uintptr_t)patched) < 0) return -1;
    return 0;
}
...
static int restore_breakpoint(pid_t pid, uint64_t addr, uint32_t orig_insn) {
    ...
    long word = ptrace(PTRACE_PEEKTEXT, pid, (void *)(uintptr_t)addr, NULL);
    if (word == -1 && errno != 0) return -1;
    uint64_t restored = ((uint64_t)word & 0xFFFFFFFF00000000ULL) | (uint64_t)orig_insn;
    if (ptrace(PTRACE_POKETEXT, pid, (void *)(uintptr_t)addr,
               (void *)(uintptr_t)restored) < 0) return -1;
    return 0;
}
...
static int find_symbol(const char *path, const char *symname, uint64_t *out) {
    ...
    Elf64_Ehdr eh;
    if (fread(&eh, sizeof(eh), 1, f) != 1) goto out;
    ...
    if (!shdrs) goto out;
    if (fseek(f, (long)eh.e_shoff, SEEK_SET) < 0) goto out;
    if (fread(shdrs, sizeof(*shdrs), eh.e_shnum, f) != eh.e_shnum) goto out;

    int symtab_idx = -1;
    for (size_t i = 0; i < eh.e_shnum; i++) {
        if (shdrs[i].sh_type == SHT_SYMTAB) { symtab_idx = (int)i; break; }
    }
    if (symtab_idx < 0) goto out;

    int strtab_idx = (int)shdrs[symtab_idx].sh_link;
    if (strtab_idx < 0 || strtab_idx >= (int)eh.e_shnum) goto out;

    size_t strsz = (size_t)shdrs[strtab_idx].sh_size;
    strtab = malloc(strsz);
    if (!strtab) goto out;
    if (fseek(f, (long)shdrs[strtab_idx].sh_offset, SEEK_SET) < 0) goto out;
    if (fread(strtab, 1, strsz, f) != strsz) goto out;

    size_t symsz  = (size_t)shdrs[symtab_idx].sh_size;
    size_t nsyms  = symsz / sizeof(Elf64_Sym);
    syms = malloc(symsz);
    if (!syms) goto out;
    if (fseek(f, (long)shdrs[symtab_idx].sh_offset, SEEK_SET) < 0) goto out;
    if (fread(syms, sizeof(*syms), nsyms, f) != nsyms) goto out;
...
int main(int argc, char **argv) {
    ...
    if (waitpid(pid, &status, 0) < 0) { perror("waitpid(bp)"); return 1; }
    ...
    uint64_t initial_x30;
    {
        ...
        if (read_regs(pid, &r) < 0) { perror("read_regs(bp)"); return 1; }
        if (r.pc != bp_addr) {
    ...
    fprintf(stderr,
            "[tracer] reached %s — pre-pushed initial X30=0x%lx; switching to single-step\n",
            bp_name, initial_x30);
    ...
    for (;;) {
        ...
        if (WIFEXITED(status)) {
            fprintf(stderr,
                    "[tracer] tracee exited status=%d  steps=%zu  bl=%zu  ret=%zu  alerts=%zu\n",
                    WEXITSTATUS(status), step_count, bl_count, ret_count, alert_count);
        ...
        case INSN_RET: {
            ...
            if (shadow_stack_pop(&expected) < 0) {
                ...
                fprintf(stderr,
                        "[tracer] BUG: ret at 0x%lx with empty shadow stack\n",
                        pre_pc);
            ...
            if (actual != expected) {
                fprintf(stderr,
                        "\n[!!! ATTACK DETECTED] ret at 0x%lx: expected 0x%lx, got 0x%lx (depth=%zu, step=%zu)\n",
                        pre_pc, expected, actual, shadow_stack_depth(), step_count);
                alert_count++;
                ptrace(PTRACE_KILL, pid, NULL, NULL);
                waitpid(pid, &status, 0);
                fprintf(stderr,
                        "[tracer] tracee killed after detection  steps=%zu  bl=%zu  ret=%zu\n",
            ...
            if (shadow_stack_depth() == 0) {
                 ...
                 */
                fprintf(stderr,
                        "[tracer] depth=0 after ret at 0x%lx — main returned cleanly, detaching\n",
                        pre_pc);
                if (ptrace(PTRACE_DETACH, pid, NULL, NULL) < 0) {
                    perror("PTRACE_DETACH");
                    return 1;
                }
                if (waitpid(pid, &status, 0) < 0) { perror("waitpid (detach)"); return 1; }
                if (WIFEXITED(status)) {
                    fprintf(stderr,
                            "[tracer] tracee exited status=%d  steps=%zu  bl=%zu  ret=%zu  alerts=%zu\n",
                            WEXITSTATUS(status), step_count, bl_count, ret_count, alert_count);
